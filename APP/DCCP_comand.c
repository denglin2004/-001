...
DCCP_Temp_Typedef g_dccp_temp = {0};//存储彩屏参数设置的临时变�?
...
static void Car_Dir_Control(uint8_t idx, uint8_t dir, uint8_t cmdParam)
{
    ...
    // 按钮按下事件：只�?�???一次收�??0x01时执行一次，防�?�循�??重�?�触�??
    ...
    // 按钮松开事件：只�?�???一次收�??0x00时执行一次，防�?�循�??重�?�触�??
...
void DCCP_comand_process(void)  
{
    ...
    if(DCCP_Rec.sDCCP.Frame_ImageID==0x01)
    {
        ...
        if(g_car.Run_S_X && g_car.Run_S_Y)
		{
            ...
            {
                g_s_task_req.step_x    = g_car.Run_S_X - 48;  // ASCII�?数字
                ...
                g_s_task_req.trigger   = 1;                    // 请求�?动S任务
