...
typedef struct {
    ...
    uint8_t  grind_get_blance_sent;           // 打磨指令已发送标�??
...
typedef struct {
    // —�? �??共变�?? —�?
    uint8_t           Step_Total;      // 指令下方的X方向的打�?�??�数  
...
typedef struct {
    ...
    uint8_t       step_x;        // X轴�?�数
    uint8_t       loop_y;        // Y轴循�?�?
    uint8_t       foc_speed;     // FOC�?�?
    uint8_t       car_speed;     // 底盘速度
    uint16_t      lift_high;     // 升降高度
    uint8_t       trigger;       // 触发标志: 1=请求�?�?
} S_TaskRequest_t;
